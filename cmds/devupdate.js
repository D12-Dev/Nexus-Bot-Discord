...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("dev update command ran!")
    await DevUpdateCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "DevUpdateCommand",
    trigger: "acm",
    aliases: []
}
async function DevUpdateCommand(receivedMessage, args){
  try{
  Modules.findOne({Type: "Modules", GuildId: receivedMessage.guild.id}).exec(async function(err,check){
    if(check.devtracker == false){
      return;
    }
    else{
  ...
  let devchannel = await client.channels.cache.get("678393450210656267");
  ...
  if (args <= 0)return await receivedMessage.channel.send("Invalid amount of arguments - Usage: .devupdate <Message> ");
  console.log(args[0])
  let message = args.slice(0).join(' ');
  var str = await receivedMessage.member.user.tag;
  var res = str.split("#")[0];
  let devupdate = new Discord.MessageEmbed()
          .setTitle("**🛠️  Development Update**")
          .setThumbnail(receivedMessage.member.user.displayAvatarURL)
          .setDescription(res + " has made a development update...")
          .addField("Update: ", message , true)
          .addField("‎‏‏‎ ‎", "Was this helpful? Please vote below.")
          .setAuthor(client.user.username)
          .setColor(0x4f007e)
          .setFooter(res + " made this update • © Nexus ", client.user.avatarURL)
          .setTimestamp(new Date())
          let reacttomessage = await devchannel.send(devupdate);
          reacttomessage.react(`✅`)
          .then(() => reacttomessage.react(`❌`))
