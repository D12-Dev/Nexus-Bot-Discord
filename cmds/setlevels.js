...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("set level command ran!")
    await SetLevelCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "SetLevelCommand",
    trigger: "acm",
    aliases: []
}
async function SetLevelCommand(receivedMessage, args) {
  try{
    ...
    let UserID = User.id;
    await Levels.setLevel(UserID, GuildID, Amount);
    await receivedMessage.channel.send(User + " was set to level " + Amount);
  }
    })
  }catch(err){
    let botlogschannel = await client.channels.cache.get("873587394224459818");
