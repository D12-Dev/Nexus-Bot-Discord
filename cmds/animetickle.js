...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("AnimeTickleCommand command ran!")
    await AnimeTickleCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "AnimeTickleCommand",
    trigger: "acm",
    aliases: []
}
async function AnimeTickleCommand(receivedMessage, args) {
  try{
    ...
    request.get(`https://nekos.life/api/v2/img/tickle`, {
    ...
    var link = JsonBody.url
      ...
      if(!Target){
        let ActionDoc = await Actions.findOne({Type: "Tickles", UsersID: receivedMessage.member.user.id})
        var AmountOfAction;
        ...
        .setAuthor(`tickle tickle ${receivedMessage.member.displayName}...`)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} tickles. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
      else{
        let ActionDoc = await Actions.findOne({Type: "Tickles", UsersID: Target.user.id})
        var AmountOfAction;
        if(!ActionDoc){
          let ActionDocCount = new Actions({
            _id: mongoose.Types.ObjectId(),
        ...
        var ImageEmbed = new Discord.MessageEmbed()
        .setAuthor(`${receivedMessage.member.displayName} tickles ${Target.displayName}... UwU `)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} tickles. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
