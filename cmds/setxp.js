...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("set xp command ran!")
    await SetXPCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
async function SetXPCommand(receivedMessage, args) {
  try{
    ...
    await receivedMessage.channel.send(User + " was set to " + Amount + " xp.");
  }
    })
  }catch(err){
    let botlogschannel = await client.channels.cache.get("873587394224459818");
  ...
  //General channel id (General shadow and summer): 571438453682nex733071
