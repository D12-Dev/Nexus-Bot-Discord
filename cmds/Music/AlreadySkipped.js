...
module.exports.run = async (msg, Client) => {
  ...
  try{
    console.log("Already song skip check util ran.")
    let HasSkipped = await AlreadySkipsFunction(msg)
    return HasSkipped
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "AlreadySkippedSkipsUtil"
}
async function AlreadySkipsFunction(msg){
    const SkipsOnSong = await Skips.get(msg.guild.id)
    // console.log(SkipsOnSong)
    if(!SkipsOnSong){
        // console.log("They haven't skipped yet!")
        return false
    }
    if(SkipsOnSong.includes(msg.member.user.id)){
        return true
    }
    else{
        return false
