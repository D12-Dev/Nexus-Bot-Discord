...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    ...
    await AnimeInsultCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "AnimeInsultCommand",
    trigger: "acm",
    aliases: []
}
async function AnimeInsultCommand(receivedMessage, args) {
  try{
    ...
    request.get(`https://nekos.life/api/v2/img/baka`, {
    ...
    var link = JsonBody.url
      ...
      if(!Target){
        let ActionDoc = await Actions.findOne({Type: "Insults", UsersID: receivedMessage.member.user.id})
        var AmountOfAction;
        ...
        .setAuthor(`You baka ${receivedMessage.member.displayName}!`)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} insults. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
      else{
        let ActionDoc = await Actions.findOne({Type: "Insults", UsersID: Target.user.id})
        var AmountOfAction;
        if(!ActionDoc){
          let ActionDocCount = new Actions({
            _id: mongoose.Types.ObjectId(),
        ...
        var ImageEmbed = new Discord.MessageEmbed()
        .setAuthor(`${receivedMessage.member.displayName} insults ${Target.displayName}, How mean...`)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} insults. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
