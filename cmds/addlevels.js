...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    ...
    await AddLevelsCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "AddLevelsCommand",
    trigger: "acm",
    aliases: []
}
async function AddLevelsCommand(receivedMessage, args) {
  try{
    Modules.findOne({Type: "Modules", GuildId: receivedMessage.guild.id}).exec(async function(err,check){
      if(check.xp == false){
        ...
        if(receivedMessage.content.length > 1800){
          return await receivedMessage.channel.send("Please make sure you message is below 1800 characters")
        }
    ...
    let GuildID = receivedMessage.guild.id;
    let User = receivedMessage.mentions.members.first();
    if (!Amount) {
      return await receivedMessage.channel.send("Invalid amount of levels was given");
    }
    if (!User) {
    ...
    await receivedMessage.channel.send("Added " + Amount + " levels to" + User);
  }
    })
  }catch(err){
    if(!receivedMessage.channel.permissionsFor(receivedMessage.guild.me).has("EMBED_LINKS"))return
