...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("add xp command ran.")
    await AddXPCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
async function AddXPCommand(receivedMessage, args) {
  try{
    ...
    await receivedMessage.channel.send("Added " + Amount + " xp to" + User);
  }
    })
  }catch(err){
    if(!receivedMessage.channel.permissionsFor(receivedMessage.guild.me).has("EMBED_LINKS"))return
