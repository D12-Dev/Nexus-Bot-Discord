...
module.exports.run = async (receivedMessage, args, Client) => {
    ...
    try{
      console.log("mute all in vc command ran!")
      await MuteAllInVcCommand(receivedMessage, args)
    }catch(err){
      console.log(err)
...
module.exports.help = {
    name: "MuteAllInVcCommand",
    trigger: "acm",
    aliases: []
}

async function MuteAllInVcCommand(receivedMessage, args){
    let Vc;
    ...
    await receivedMessage.channel.send(`Muting ${Vc.members.size} members in voice channel: ${Vc}.`)
    await Vc.members.forEach(async (member) => {
        if(member == receivedMessage.member)return 
        if(member.voiceChannel){
            try{
                await member.setMute(true, "[Auto Nexus] Muting all people in current vc! By " + receivedMessage.member.user.tag + ".");
                MemberMutedCount + 1
            }catch(err){
    ...
    await receivedMessage.channel.send(`Muted ${MemberMutedCount} members in voice channel: ${Vc}.`)
}