...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("Hentai command ran!")
    await HentaiCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
async function HentaiCommand(receivedMessage, args){
    Modules.findOne({Type: "Modules", GuildId: receivedMessage.guild.id}).exec(async function(err,check){
        if(check.nsfw == false){
          return;
    }
    let Target = await receivedMessage.mentions.members.first()
    if(!Target){
      receivedMessage.channel.send("You need to mention someone to fuck silly goose. Fuck <User> (Preferably your bf :<)")
      return
    }
    ...
    .end(async(err, response) => {
        ...
        .setAuthor(`${receivedMessage.member.displayName} fucked ${Target.displayName}. Spicy!!`)
        .setImage(response.body.message)
        .setFooter(`Powered by neokbot.com.`)
        await receivedMessage.channel.send({embeds: [ImageEmbed]});
