...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("Xp ban command ran!")
    await XpBanCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
async function XpBanCommand(receivedMessage, args){
  try{
  Modules.findOne({Type: "Modules", GuildId: receivedMessage.guild.id}).exec(async function(err,check){
    ...
    if(args == 0)return await receivedMessage.channel.send("Invalid amount of arguments! Usage: .xpban <user> <reason>")
  let member = await receivedMessage.mentions.members.first() || receivedMessage.guild.members.get(args[0]);
  ...
  console.log(reason)
  var str = member.user.tag;
  var res = str.split("#")[0];
  XpBans.findOne({UserID: UserId, GuildId: receivedMessage.guild.id}).exec(async function(err,XPdocument){
    if(XPdocument === null){
        const xpbanuser = new XpBans({
          _id: mongoose.Types.ObjectId(),
          Type: "XP-Ban",
          UserID: UserId,
          UserName: member.user.tag,
          PunishmentGiverID: receivedMessage.member.id,
          PunishmentGiverUsername: receivedMessage.member.user.tag,
          Reason: reason,
          Time: "Time:",
          GuildId: receivedMessage.guild.id,
        })
        await xpbanuser.save()
        ...
        await BotLogsChannelData.send(`${res} has been xp banned by ${receivedMessage.author.tag} reason: ${reason}`)
        return await receivedMessage.channel.send(res + " is now XP banned, with the reason: " + reason)
        })
    }
    else{
      await receivedMessage.channel.send("This user is already xp banned! - To unban this user do .xpunban " + res + " <Reason>")
    }
