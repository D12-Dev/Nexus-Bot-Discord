...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("MemberCountServerStats command ran!")
    await MemberCountServerStats(receivedMessage)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "MemberCountServerStatsCommand",
    trigger: "acm",
    aliases: []
}
async function MemberCountServerStats(receivedMessage){
    if(receivedMessage.guild.id != "676874632078426180")return await receivedMessage.channel.send("Only admins in the nexus dev discord can see server stats...")
    ...
    let MemberCountServerStatsEmbed = new Discord.MessageEmbed()
    .setTitle("**Server member counts!**")
    .setDescription("These are the top 10 server member counts!")
    .setAuthor(client.user.username)
    .setColor(0x0080ff)
    .setTimestamp(new Date())
    .setFooter("• © Nexus")
    let GuildEmbedArray = []
    let PageNumber = 1
    let MaxPageNumber = Math.ceil(client.guilds.size / 10)
    client.guilds.forEach(async(guild) => {
        GuildEmbedArray.push(`\`${guild.name}\` || This server has \`${guild.members.filter((m) => !m.user.bot).size}\` members currently in it.`)
      })
      if(GuildEmbedArray.length > 10){
        for(var x = 0;x < 10;x++){
            MemberCountServerStatsEmbed.addField(`⠀`, GuildEmbedArray[x])
        }
      }
      else{
    ...
    let SentEmbed = await receivedMessage.channel.send(MemberCountServerStatsEmbed)
    ...
    await backwards.on('collect',async r => {
      ...
      if(PreviousPageNum * 10 >= GuildEmbedArray.length){
        ...
        await SentEmbed.edit(TurnPageEmbed)
      }
      else{
        var i = 0
        index = ((PageNumber - 1) * 10)
