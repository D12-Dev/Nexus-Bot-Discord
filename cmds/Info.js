...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    ...
    await InfoAboutBotCommand(receivedMessage)
  }catch(err){
    console.log(err)
...
async function InfoAboutBotCommand(receivedMessage){
  try{
    ...
    var AmountOfNexusMembers = 0
    client.guilds.forEach(guild => {
      AmountOfNexusMembers = AmountOfNexusMembers + guild.members.filter((m) => !m.user.bot).size
    });
    let InfoEmbed = new Discord.MessageEmbed()
    .setTitle("Info about Nexus!")
    .setDescription("✨**These are all the contributers for the nexus discord bot!**✨\n\n**D12#0001** - Main contributer, made all main features.\n\n **Shadowhawk#8720** - Contributer, Created concepts for the discord bot and coded some features.\n")
    .addField("Server Count:","Nexus is currently in\n `" + client.guilds.size + " servers!`", true)
    .addField("Discord link:", "To add the bot to your guild, click [here](https://discordapp.com/api/oauth2/authorize?client_id=673285454724071456&permissions=8&scope=bot).", true)
    .addField("Updates", "Last update: `" + LastUpdateDate + "`.", true)
    .addField(`Members:`, "There are currently `" + receivedMessage.guild.members.filter((m) => !m.user.bot).size + "` members in this server!")
    .addField(`Members that use Nexus:`, `There are currently \`${AmountOfNexusMembers}\` members that use Nexus.`)
