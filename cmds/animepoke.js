...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("AnimePokeCommand command ran!")
    await AnimePokeCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "AnimePokeCommand",
    trigger: "acm",
    aliases: []
}
async function AnimePokeCommand(receivedMessage, args) {
  try{
    ...
    request.get(`https://nekos.life/api/v2/img/poke`, {
    ...
    var link = JsonBody.url
      ...
      if(!Target){
        let ActionDoc = await Actions.findOne({Type: "Pokes", UsersID: receivedMessage.member.user.id})
        var AmountOfAction;
        ...
        .setAuthor(`Poke poke ${receivedMessage.member.displayName}!`)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} pokes. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
      else{
        let ActionDoc = await Actions.findOne({Type: "Pokes", UsersID: Target.user.id})
        var AmountOfAction;
        if(!ActionDoc){
          let ActionDocCount = new Actions({
            _id: mongoose.Types.ObjectId(),
        ...
        var ImageEmbed = new Discord.MessageEmbed()
        .setAuthor(`${receivedMessage.member.displayName} pokes ${Target.displayName}, I wonder what they want... `)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} pokes. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
