...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("Hug command ran!")
    await HugImageCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "HugImageCommand",
    trigger: "acm",
    aliases: []
}
async function HugImageCommand(receivedMessage, args){
  try{
    ...
    request.get(`https://nekos.life/api/v2/img/hug`, {
    ...
    if(!Target){
      let ActionDoc = await Actions.findOne({Type: "Hugs", UsersID: receivedMessage.member.user.id})
      var AmountOfAction;
      if(!ActionDoc){
      ...
      .setAuthor(`Here ${receivedMessage.member.displayName} have a hug... UwU`)
      .setImage(link)
      .setFooter(`You have now recieved ${AmountOfAction} hugs. OwO`)
      return await receivedMessage.channel.send({embeds: [ImageEmbed]});
    }
    else{
      let ActionDoc = await Actions.findOne({Type: "Hugs", UsersID: Target.user.id})
      var AmountOfAction;
      if(!ActionDoc){
        let ActionDocCount = new Actions({
          _id: mongoose.Types.ObjectId(),
      ...
      var ImageEmbed = new Discord.MessageEmbed()
      .setAuthor(`${receivedMessage.member.displayName} gave ${Target.displayName} a hug... OwO`)
      .setImage(link)
      .setFooter(`You have now recieved ${AmountOfAction} hugs. :3`)
      return await receivedMessage.channel.send({embeds: [ImageEmbed]});
    }
  })
  }catch(err){
