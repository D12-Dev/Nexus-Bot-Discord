...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("JademRaps command ran!")
    await JademRapsCommand(receivedMessage)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "JademRapsCommand",
    trigger: "acm",
    aliases: []
}
async function JademRapsCommand(receivedMessage) {
    try{
      await receivedMessage.author.send("Please do " + PREFIX + "p https://youtu.be/ww0Q6Zc1rm8 to add a special song to the music queue!")
    }catch(err){
        ...
        .setTimestamp(new Date())
        return await receivedMessage.channel.send(failedtosend)
    }
  }
