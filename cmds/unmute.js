...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("unmute command ran!")
    await UnmuteCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
async function UnmuteCommand(receivedMessage, args) {
  try{
  ...
  if (!member.kickable)
    return await receivedMessage.reply(
      "I cannot unmute this user! Do they have a higher role? Do I have mute permissions?"
    );
  ...
  await receivedMessage.channel.send(
    `${member.user.tag} has been unmuted by ${receivedMessage.author.tag} reason: ${reason}`
  );
  try{
    await receivedMessage.mentions.members
    .first()
    .send(
      `You have been unmuted by ${receivedMessage.author.tag} with the reason: ${reason}`
    );
  }catch(err){
  ...
  await BotLogsChannelData.send(receivedMessage.author.tag + " unmuted " + member.user.tag + " reason:" + reason)
    })
    }
  })
}catch(err){
