...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("AnimeSlapCommand command ran!")
    await AnimeSlapCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "AnimeSlapCommand",
    trigger: "acm",
    aliases: []
}
async function AnimeSlapCommand(receivedMessage, args) {
  try{
    ...
    request.get(`https://nekos.life/api/v2/img/slap`, {
    ...
    var link = JsonBody.url
      ...
      if(!Target){
        let ActionDoc = await Actions.findOne({Type: "Slaps", UsersID: receivedMessage.member.user.id})
        var AmountOfAction;
        ...
        .setAuthor(`Bad ${receivedMessage.member.displayName}!`)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} slaps. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
      else{
        let ActionDoc = await Actions.findOne({Type: "Slaps", UsersID: Target.user.id})
        var AmountOfAction;
        if(!ActionDoc){
          let ActionDocCount = new Actions({
            _id: mongoose.Types.ObjectId(),
        ...
        var ImageEmbed = new Discord.MessageEmbed()
        .setAuthor(`${receivedMessage.member.displayName} slapped ${Target.displayName}. OwO They must have been very naughty. `)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} slaps. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
