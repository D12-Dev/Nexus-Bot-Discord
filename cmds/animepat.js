...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    ...
    await AnimePatCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "AnimePatCommand",
    trigger: "acm",
    aliases: []
}
async function AnimePatCommand(receivedMessage, args) {
  try{
    ...
    request.get(`https://nekos.life/api/v2/img/pat`, {
    ...
    var link = JsonBody.url
      ...
      if(!Target){
        let ActionDoc = await Actions.findOne({Type: "Pats", UsersID: receivedMessage.member.user.id})
        var AmountOfAction;
        ...
        .setAuthor(`There there, ${receivedMessage.member.displayName}... O-O`)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} pats. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
      else{
        let ActionDoc = await Actions.findOne({Type: "Pats", UsersID: Target.user.id})
        var AmountOfAction;
        if(!ActionDoc){
          let ActionDocCount = new Actions({
            _id: mongoose.Types.ObjectId(),
        ...
        var ImageEmbed = new Discord.MessageEmbed()
        .setAuthor(`${receivedMessage.member.displayName} pats ${Target.displayName}, How cute!`)
        .setImage(link)
        .setFooter(`You have now recieved ${AmountOfAction} pats. :3`)
        return await receivedMessage.channel.send({embeds: [ImageEmbed]});
      }
