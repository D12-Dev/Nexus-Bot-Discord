...
module.exports.run = async (receivedMessage, Client) => {
  ...
  try{
    console.log("ai command ran.")
    await AiResponse(receivedMessage)
  }catch(err){
    console.log(err)
...
async function AiResponse(receivedMessage){
  try{
  ...
  if(receivedMessage.mentions.users.has(client.user.id)){
    text = await receivedMessage.content.toLowerCase().replace("<@!" + client.user.id + ">",'')
  }
  else{
    text = await receivedMessage.content.toLowerCase().replace('nexus','')
  }
  await cleverbot(text).then(async response => {
    await receivedMessage.channel.sendTyping();
        setTimeout(async() => {
          await receivedMessage.channel.send(response).catch(console.error);
