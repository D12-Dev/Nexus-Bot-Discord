...
module.exports.run = async (receivedMessage, args, Client) => {
    ...
    try{
      console.log("mute command ran!")
      await MuteCommand(receivedMessage, args)
    }catch(err){
      console.log(err)
...
async function MuteCommand(receivedMessage, args) {
  try{
  ...
  if (!member.kickable)return await receivedMessage.reply("I cannot mute this user! Do they have a higher role?");
 // console.log(muterole)
 try{
  await member.addRole(muterole)
 }catch(err){
   return await receivedMessage.channel.send("I cannot mute this user! Do they have a higher role?")
 }
  ...
  await receivedMessage.channel.send(
    `${member.user.tag} has been muted by ${receivedMessage.author.tag} because: ${reason}`
  );
  try{
    await receivedMessage.mentions.members
    .first()
    .send(
      `You have been muted by ${receivedMessage.author.tag} with the reason: ${reason}`
    );
  }catch(err){
  ...
  await BotLogsChannelData.send(receivedMessage.author.tag + " muted " + member.user.tag + " reason:" + reason)
    })
  }
})
  }catch(err){
