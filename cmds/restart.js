...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("restart command ran!")
    await ResetBot(receivedMessage)
  }catch(err){
    console.log(err)
...
async function ResetBot(receivedMessage) {
  try{
    ...
    if(receivedMessage.guild.id === "676874632078426180"){
      await receivedMessage.channel.send('Restarting bot...')
      .then(async () => await client.destroy())
      .then(async() => await client.login(TOKEN));
    }
    else{
      return await receivedMessage.channel.send("Only admins in the nexus dev discord can restart the bot.Please contact 🔥 D12 🔥#0001, if something critical has occured! https://discord.gg/wEdRXDM")
    }
  }catch(err){
    let botlogschannel = await client.channels.cache.get("873587394224459818");
