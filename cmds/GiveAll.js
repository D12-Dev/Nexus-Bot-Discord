...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("Give all command ran!")
    await GiveAll(receivedMessage, args)
  }catch(err){
    console.log(err)
...
async function GiveAll(receivedMessage, args){
  try{
    ...
    if(guild.me.highestRole.comparePositionTo(Role) > 0){
      await AddSlowlyRoles(receivedMessage, guild, Role)
      return
    }
...
async function AddSlowlyRoles(receivedMessage, guild, role){
  ...
  for(var x = 0;x < MemberArray.length;x++){
    ...
    try{
      await MemberArray[x].addRole(role)
    }catch(err){
      console.log("Failed to add role!")
    }
  }
  return await receivedMessage.channel.send(`Gave \`${MemberArray.length}\` members the role ${role}.`);
}
