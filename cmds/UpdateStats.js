...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("Update stats command ran!")
    await UpdateStatsCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
async function UpdateStatsCommand(receivedMessage, args){
  try{
    ...
    if(receivedMessage.guild.id != "676874632078426180")return await receivedMessage.channel.send("Only admins in the nexus dev discord can post an update message for the bot.Please contact 🔥 D12 🔥#0001, if something critical has occured! https://discord.gg/wEdRXDM")
    ...
    return await receivedMessage.channel.send(`There are currently ${ChannelCount} guilds with Nexus in its voice channel.`)
  }catch(err){
    let botlogschannel = await client.channels.cache.get("873587394224459818");
