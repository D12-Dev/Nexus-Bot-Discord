...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("Remove xp command ran!")
    await RemoveXPCommand(receivedMessage, args)
  }catch(err){
    console.log(err)
...
module.exports.help = {
    name: "removeXPCommand",
    trigger: "acm",
    aliases: []
}
async function RemoveXPCommand(receivedMessage, args) {
  try{
    ...
    await receivedMessage.channel.send("Removed " + positiveAmount + " xp from" + User);
  }
  })
}catch(err){
  let botlogschannel = await client.channels.cache.get("873587394224459818");
