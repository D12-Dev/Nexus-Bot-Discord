...
module.exports.run = async (receivedMessage, args, Client) => {
  ...
  try{
    console.log("Nexus version command ran!")
    await NexusVersionCommand(receivedMessage)
  }catch(err){
    console.log(err)
...
async function NexusVersionCommand(receivedMessage){
  try{
    return await receivedMessage.author.send("The current version of Nexus is `" + NexusVersion + "`")
  }catch(err){
    return
  }
}
